...
describe('Fill out New Diploma Form', () => {
    ...
    it('Fill out new diploma form', () => {
        cy.get('.v-form > :nth-child(1) > .v-input__control > .v-input__slot > .v-text-field__slot > input').type('Its Friday!!!')
        cy.get('textarea').type('Epic Dance Party')
        cy.get('.v-form > .v-text-field--enclosed > .v-input__control > .v-input__slot > .v-select__slot').click()//lists of prereqs
        cy.get('.menuable__content__active > .v-select-list > .v-list > :nth-child(1) > .v-list__tile').click()//alone low investment
        cy.get('.v-form > .v-text-field--enclosed > .v-input__control > .v-input__slot > .v-select__slot > .v-select__selections').click()
        cy.get('.menuable__content__active > .v-select-list > .v-list > :nth-child(2) > .v-list__tile').click()
    })
    it('Clear new Diploma form', () => {
        cy.get('.v-card__actions > .primary--text').click()
    })
    it('Cancel new diploma', () => {
        cy.get('.v-dialog__content--active > .v-dialog > .v-card > .v-card__actions > .secondary--text').click()
    })
})
...
describe('Add Diploma', () => {
    it('Filling out new diploma form', () => {
        cy.get('[data-cy=diplomas-table-new]').click()
        cy.get('.v-form > :nth-child(1) > .v-input__control > .v-input__slot > .v-text-field__slot > input').type(new_diploma)
        cy.get('textarea').type('Epic Dance Party')
        cy.get('.v-form > .v-text-field--enclosed > .v-input__control > .v-input__slot > .v-select__slot').click()//lists of prereqs
        cy.get('.menuable__content__active > .v-select-list > .v-list > :nth-child(1) > .v-list__tile').click()//alone low investment
        cy.get('.v-form > .v-text-field--enclosed > .v-input__control > .v-input__slot > .v-select__slot > .v-select__selections').click()
