  ...
  <div>
    <v-toolbar class="pa-1">
      <v-layout align-center justify-space-between fill-height>
        <v-flex md3>
          <v-toolbar-title>{{ $t("actions.tooltips.take-attendance") }}</v-toolbar-title>
        </v-flex>
        <v-flex md2>
          <v-text-field
            v-model="search"
      ...
      methods: {
        ...
        submitSelectedPeople(){
          ...
          for(let i = 0; i < newAttendance.length; i ++){
            this.$http
              .patch(`/api/v1/groups/meetings/${meetingId}/attendances/${newAttendance[i]}`)
              .then(resp => {
          ...
          if (this.selected.length < currentStore.length){
            ...
            for(let i = 0; i < missingId.length; i ++){
              if(currentStore.includes(missingId[i]) === true){
                this.$http
                  .delete(`/api/v1/groups/meetings/${meetingId}/attendances/${missingId[i]}`)
                  .then(resp => {
                    eventBus.$emit('message', { content:this.$t("groups.messages.participant-added") });
                    this.allMeetingAttendance();
                  })
