  ...
  methods: {
    load() {
      ...
      promises.push(
        this.$http.get(
          ...
          this.students = resp.data.filter(student => student.active && student.confirmed);
        })
      );
    ...
    applyAttendance(attendance) {
      this.students.forEach(student => {
        student.present = 
          !!attendance.find(record => record.studentId == student.id);
      });
    },
    ...
    save() {
      this.saving = true;
      let attendance =
        this.students.filter(student => student.present)
