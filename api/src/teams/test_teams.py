...
def test_create_team(auth_client):
    ...
    queried_team = auth_client.sqla.query(Team).filter(
        Team.id == resp.json["id"]).first()
...
def test_read_all_teams(auth_client):
    ...
    filtered_teams = auth_client.get(
        url_for(
            'teams.read_all_teams',
            return_group="all",
            desc="awesome",
            sort='description_desc')).json
    ...
    queried_active_teams_count = auth_client.sqla.query(
        Team).filter(Team.active).count()
    ...
    for team in active_teams:
        queried_team = auth_client.sqla.query(
            Team).filter(Team.id == team["id"]).first()
    ...
    all_teams = auth_client.get(
        url_for(
            'teams.read_all_teams',
            return_group="all")).json
    # THEN we should have the same number
    assert len(all_teams) == count
    # WHEN we ask for all inactive teams
    inactive_teams = auth_client.get(
        url_for(
            'teams.read_all_teams',
            return_group="inactive")).json
    queried_inactive_teams_count = auth_client.sqla.query(
        Team).filter(Team.active == False).count()
...
def test_read_all_team_members(auth_client):
    ...
    for team in teams:
        members = auth_client.sqla.query(TeamMember).filter(
            TeamMember.team_id == team.id).all()

        resp = auth_client.get(
            url_for(
                'teams.read_all_team_members',
                team_id=team.id))
        # THEN we expect a correct status code
        assert resp.status_code == 200

        # THEN we expect for each member of the team, the current team's id is
        # in the member's "teams"
...
def test_add_team_member(auth_client):
    ...
    # THEN we expect the right status code
    assert resp.status_code == 200
    # THEN we expect the correct count of linked team and member in the
    # database
    count = auth_client.sqla.query(TeamMember).filter(
        TeamMember.team_id == team_id,
        TeamMember.member_id == member_id).count()
    assert count == 1
...
def test_delete_team_member(auth_client):
    ...
    team_member = auth_client.sqla.query(
        TeamMember).filter(TeamMember.active).first()
    count = auth_client.sqla.query(
        TeamMember).filter(TeamMember.active).count()
    ...
    assert 1 == auth_client.sqla.query(TeamMember).filter(
        TeamMember.active == False,
        TeamMember.team_id == team_member.team_id,
        TeamMember.member_id == team_member.member_id).count()
    # THEN We expect the correct count of link in the database
    new_count = auth_client.sqla.query(
        TeamMember).filter(TeamMember.active).count()
