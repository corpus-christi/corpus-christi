...
def test_create_asset(auth_client):
    ...
    queried_asset = auth_client.sqla.query(Asset).filter(
        Asset.id == resp.json["id"]).first()
    for attr in new_asset:
        assert new_asset[attr] == queried_asset.__dict__[attr]


@pytest.mark.skip(
    reason="Currently will fail, since the api does not check for invalid location")
...
def test_read_all_assets(auth_client):
    ...
    filtered_assets = auth_client.get(
        url_for(
    ...
    queried_active_assets_count = auth_client.sqla.query(
        Asset).filter(Asset.active).count()
    ...
    for asset in active_assets:
        queried_asset = auth_client.sqla.query(
            Asset).filter(Asset.id == asset["id"]).first()
    ...
    inactive_assets = auth_client.get(
        url_for(
            'assets.read_all_assets',
    ...
    all_assets_matching_desc = auth_client.get(
        url_for('assets.read_all_assets', desc='c')).json
    # THEN all results should match that description
    for asset in all_assets_matching_desc:
        assert 'c' in asset['description'].lower()
    all_assets_matching_loc_id = auth_client.get(
        url_for('assets.read_all_assets', location_id=1)).json
    # THEN all results should match that description
