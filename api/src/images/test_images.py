...
def test_download_image(auth_client):
    ...
    for image in images:
        ...
        resp = auth_client.get(url_for('images.download_image', image_id = image.id))
...
def test_download_invalid_image(auth_client):
    ...
    resp = auth_client.get(url_for('images.download_image', image_id = fake_image_id))
...
def test_update_image(auth_client):
    ...
    for image in images:
        ...
        resp = auth_client.patch(url_for('images.update_image', image_id=image.id), json=payload)
        # THEN we expect each of them to correspond grab an image in the file system
...
def test_delete_image(auth_client):
    ...
    # WHEN we ask for the events one by one
    for image in images:
        resp = auth_client.delete(url_for('images.delete_image', image_id = image.id))
        # THEN we expect a 204 and for the image to not be in the db or file system
        assert resp.status_code == 204
        assert len(auth_client.sqla.query(Image).filter_by(id=image.id).all()) == 0
...
def test_delete_invalid_image(auth_client):
    ...
    fake_image_id = -1
    # WHEN we ask for an invalid image
    resp = auth_client.delete(url_for('images.delete_image', image_id = fake_image_id))
    # THEN we expect to get a 404
    assert resp.status_code == 404