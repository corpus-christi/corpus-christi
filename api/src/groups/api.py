...
from src.shared.helpers import modify_entity, get_all_queried_entities, log
...
def create_group_type():
    try:
        ...
        return log(jsonify(err.messages), 422)

    new_group_type = GroupType(**valid_group_type)
    db.session.add(new_group_type)
    db.session.commit()
    return log(jsonify(group_type_schema.dump(new_group_type)), 201)
...
def read_one_group_type(group_type_id):
    ...
    if not group_type:
        return log(jsonify(f"GroupType with id #{group_type_id} does not exist."), 404)

    return log(jsonify(group_type_schema.dump(group_type)))
...
def read_all_group_types():
    ...
    try:
        ...
        return log(jsonify(e.message), e.code)
    return log(jsonify(group_type_schema.dump(group_types, many=True)))
...
def update_group_type(group_type_id):
    ...
    return log(modify_entity(GroupType, group_type_schema, group_type_id, valid_attributes))
...
def delete_group_type(group_type_id):
    ...
    return log("Deleted successfully", 204)
...
def create_group():
    ...
    # when the foreign key given is invalid
        ...
        return log('Payload contains an invalid group type', 404)

    return log(jsonify(group_schema.dump(new_group)), 201)
...
def read_all_groups():
    ...
    try:
        ...
        return log(jsonify(e.message), e.code)
    group_schema = GroupSchema()
    return log(jsonify(group_schema.dump(groups, many=True)), 200)
...
def read_one_group(group_id):
    ...
    if group is None:
        return log(jsonify(f"Group with id {group_id} does not exist"), 404)
    return log(jsonify(group_schema.dump(group)), 200)
...
def update_group(group_id):
    ...
    return log(modify_entity(Group, group_schema, group_id, valid_attributes))
...
def create_manager_type():
    ...
    db.session.commit()
    return log(jsonify(manager_type_schema.dump(new_manager_type)), 201)
...
def read_one_manager_type(manager_type_id):
    ...
    if not manager_type:
        return log(jsonify(f"ManagerType name: #{manager_type.name} does not exist."), 404)

    return log(jsonify(manager_type_schema.dump(manager_type)))
...
def read_all_manager_types():
    ...
    try:
        ...
        return log(jsonify(e.message), e.code)
    return log(jsonify(manager_type_schema.dump(manager_types, many=True)))
...
def update_manager_type(manager_type_id):
    ...
    return log(modify_entity(ManagerType, manager_type_schema, manager_type_id, valid_attributes))
...
def create_manager(group_id):
    ...
    if db.session.query(Manager).filter_by(person_id=person_id, group_id=group_id).first():
        ...
        return log(jsonify(f"Manager with group_id #{group_id} and person_id #{person_id} already exists"), 409)
    ...
    try:
        ...
        return log('Payload contains an invalid group/person key', 404)
    return log(jsonify(manager_schema.dump(new_manager)), 201)
...
def read_all_managers(group_id):
    ...
    try:
        ...
        return log(jsonify(e.message), e.code)
    return log(jsonify(manager_schema.dump(managers, many=True)))
...
def read_one_manager(group_id, person_id):
    ...
    if manager is None:
        return log(jsonify(f"Manager with group name: #{group.name} and person's username #{person.username} does not exist"), 404)
    return log(jsonify(manager_schema.dump(manager)))
...
def update_manager(group_id, person_id):
    ...
    return log(jsonify(manager_schema.dump(manager)), 200)
...
def create_meeting():
    ...
    # when the foreign key given is invalid
        ...
        return log('Payload contains an invalid address/group key', 404)

    return log(jsonify(meeting_schema.dump(new_meeting)), 201)
...
def read_all_meetings():
    ...
    try:
        ...
        return log(jsonify(e.message), e.code)
    meeting_schema = MeetingSchema()
    return log(jsonify(meeting_schema.dump(meetings, many=True)), 200)
...
def read_one_meeting(meeting_id):
    ...
    if meeting is None:
        return log(jsonify(f"Meeting with description: {meeting.description} does not exist"), 404)
    return log(jsonify(meeting_schema.dump(meeting)), 200)
...
def update_meeting(meeting_id):
    ...
    return log(modify_entity(Meeting, meeting_schema, meeting_id, valid_attributes))
...
def create_member(group_id):
    ...
    if db.session.query(Member).filter_by(person_id=person_id, group_id=group_id).first():
        ...
        return log(jsonify(f"Member with group_id #{group_id} and person_id #{person_id} already exists"), 409)
    ...
    try:
        ...
        return log('Payload contains an invalid group/person key', 404)
    return log(jsonify(member_schema.dump(new_member)), 201)
...
def read_all_members(group_id):
    ...
    try:
        ...
        return log(jsonify(e.message), e.code)
    return log(jsonify(member_schema.dump(members, many=True)))
...
def read_one_member(group_id, person_id):
    ...
    if member is None:
        return log(jsonify(f"Member with group name: #{group.name} and person username: #{person.username} does not exist"), 404)
    return log(jsonify(member_schema.dump(member)))
...
def update_member(group_id, person_id):
    ...
    return log(jsonify(member_schema.dump(member)), 200)
...
def create_attendance(meeting_id, person_id):
    if not db.session.query(Person).filter_by(id=person_id).first():
        return log(jsonify(f"Person with person username #{person.username} does not exist"), 404)

    if not db.session.query(Meeting).filter_by(id=meeting_id).first():
        return log(jsonify(f"Meeting with meeting description: #{meeting.description} does not exist"), 404)
    ...
    if db.session.query(Attendance).filter_by(person_id=person_id, meeting_id=meeting_id).first():
        ...
        return log(jsonify(f"Attendance with meeting name: #{meeting.name} and person username #{person.username} already exists"), 409)

    new_attendance = Attendance(meeting_id=meeting_id, person_id=person_id)
    db.session.add(new_attendance)
    db.session.commit()

    return log(jsonify(attendance_schema.dump(new_attendance)), 201)
...
def read_all_attendances(meeting_id):
    ...
    try:
        ...
        return log(jsonify(e.message), e.code)
    return log(jsonify(attendance_schema.dump(attendances, many=True)))
...
def delete_attendance(meeting_id, person_id):
    ...
    if attendance is None:
        return log(jsonify(f"Attendance with meeting name: #{meeting.name} and person username #{person.username} does not exist"), 404)
...
def add_group_images(group_id, image_id):
    ...
    if not group:
        return log(jsonify(f"Group with name: #{group.name} does not exist."), 404)

    if not image:
        return log(jsonify(f"Image with path #{image.path} does not exist."), 404)

    # If image is already attached to the group
    if group_image:
        return log(jsonify(f"Image with id#{image_id} is already attached to group with id#{group_id}."), 422)
    else:
        new_entry = ImageGroup(**{'group_id': group_id, 'image_id': image_id})
        db.session.add(new_entry)
        db.session.commit()

    return log(jsonify(f"Image with id #{image_id} successfully added to Group with id #{group_id}."), 201)
...
def put_group_images(group_id, image_id):
    ...
    if old_image_id == 'false':
        ...
        return log(jsonify({'deleted': 'No image to delete', 'posted': str(post_resp[0].data, "utf-8")}))
    else:
        del_resp = delete_group_image(group_id, old_image_id)
        post_resp = add_group_images(group_id, new_image_id)

        return log(jsonify({'deleted': del_resp[0], 'posted': str(post_resp[0].data, "utf-8")}))
...
def delete_group_image(group_id, image_id):
    ...
    if not group_image:
        return log(jsonify(f"Image with path #{image.path} is not assigned to Group with name #{group.name}."), 404)

    db.session.delete(group_image)
    db.session.commit()

    # 204 codes don't respond with any content
    return log('Successfully removed image', 204)
