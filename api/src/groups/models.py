...
class Group(Base):
    ...
    meetings = relationship('Meeting', backref='groups', lazy=True)
    members = relationship('Member', backref='groups', lazy=True)
    images = relationship('ImageGroup', backref='groups', lazy=True)
...
class Meeting(Base):
    ...
    description = Column(StringTypes.LONG_STRING, nullable=False)
    active = Column(Boolean, nullable=False, default=True)
    attendances = relationship('Attendance', backref='meetings', lazy=True)
...
class Member(Base):
    ...
    active = Column(Boolean, nullable=False, default=True)
    attendances = relationship('Attendance', backref='members', lazy=True)
