...
class Location(Base):
    ...
    images = relationship('ImageLocation', backref="location")
    active = Column(Boolean, nullable=False, default=True)
...
class Address(Base):
    ...
    meetings = relationship('Meeting', backref='address', lazy=True)
    locations = relationship('Location', backref='address', lazy=True)
    active = Column(Boolean, nullable=False, default=True)
